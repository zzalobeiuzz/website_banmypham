// làm một table banner sau đó muốn thay đổi thì dựa trên nó 
//VD: hết chương trình thì tắt mở banner khác và tự động chạy 
...
const Banner = () => {
  ...
  useEffect(() => {
    const interval = setInterval(() => {
      // setInterval là một hàm có sẵn trong JavaScript 
      // thực hiện một đoạn mã (hàm) lặp đi lặp lại sau mỗi khoảng thời gian cố định (tính bằng mili giây).
      handleNext(); 
    }, 4000);
  ...
  const handlePrev = () => {
    setActiveIndex((prevIndex) => {
      console.log(bannerImages.length); // In ra độ dài mảng
              ...
              {bannerImages.map((item, index) => (
                ...
                >
                  <div>
                    <a href="./">
                      <img
                        src={`${UPLOAD_BASE}/uploads/banners/${item.Image}`} // Đường dẫn ảnh
