  ...
  useEffect(() => {
    const fetchProduct = async () => {
      try {
        ...
        setProduct(res.data); // ✅ Chỉ setProduct
        setTimeout(() => setLoading(false), 1000); // Delay cho animation nếu có
      } catch (err) {
    ...
    fetchProduct();
  }, [id]); // ⚠️ Bỏ request ra nếu không dùng useCallback ổn định
  
  // 🧠 Khi product có dữ liệu thì mới set editFields
  useEffect(() => {
    if (product) {
      setEditFields({
