...
import useHttp from "../"; // 👉 Đường dẫn đúng với file của bạn
import "./style.scss";
...
const AddProduct = () => {
  ...
  const handleBarcodeResult = async (barcode) => {
    ...
    try {
      // Gọi API backend kiểm tra sản phẩm theo barcode
      const res = await fetch(`/api/products/check-barcode?code=${barcode}`);
      const data = await res.json();

      if (data.exists) {
        // Sản phẩm đã tồn tại
        alert(`⚠️ Sản phẩm đã tồn tại: ${data.product.name}`);
        ...
        // Sản phẩm chưa tồn tại → gán barcode vào tên sản phẩm
        setName(barcode);
      }
    } catch (error) {
      console.error("Lỗi kiểm tra sản phẩm:", error);
      alert("Không thể kiểm tra sản phẩm. Vui lòng thử lại!");
    }
