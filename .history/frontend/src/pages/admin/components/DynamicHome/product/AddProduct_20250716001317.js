...
const AddProduct = () => {
 ...
 const handleSubmitAdd = async () => {
  ...
  try {
    ...
    const processedHtml = await handleHtmlImagesBatch([intro, usage, ingredients, instructions], request);
    [intro, usage, ingredients, instructions] = processedHtml;

    // ✅ Update lại state để đồng bộ với React trước khi submit
    setProductData((prev) => ({
      ...prev,
      intro,
      usage,
  ...
  const handleImageChange = (e) => {
    ...
    if (file) {
      // Nếu file giống file cũ (theo tên và size), không cập nhật preview
      if (fileName === file.name && imageFile?.size === file.size) {
        console.log("⚠️ File trùng, không cập nhật preview");
        return;
      }
      setImageFile(file);
            ...
            <div className="input-type">
              ...
              >
                ...
                {subCategories.map((sub) => (
                  <option classkey={sub.SubCategoryID} value={sub.SubCategoryID}>{sub.SubCategoryName}</option>
                ))}
              </select>
            </div>
