...
const WizardForm = () => {
  ...
  const handleSubmitEmail = async (e) => {
    ...
    try {
      ...
      if (response.ok) {
        const fakeCode = "123456"; // hoặc generate mã thật từ backend
        setSentCode(fakeCode);
        setStep(4);
      } else {
        const data = await response.json();
        alert(data.message || "Gửi email thất bại.");
      }
    } catch (err) {
  ...
  const handleSubmit = async (e) => {
    ...
    const payload = {
      email: formData.account.email, // chỉ gửi email lên để gửi mã xác thực
    };
  
    try {
      const response = await fetch("/api/a/register", {
        method: "POST",
        headers: {
          "Content-Type": "application/json",
        },
      ...
      if (response.ok) {
        ...
        setSentCode(data.code); // dùng nếu bạn cần lưu code phía client để so sánh, nếu không thì bỏ
        setStep(4);
      } else {
