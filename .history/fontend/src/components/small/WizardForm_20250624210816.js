...
const WizardForm = () => {
  ...
  const [emailError, setEmailError] = useState(null); // chỉ cho lỗi
  ...
  const handleSubmitEmail = async (e) => {
    ...
    try {
      ...
      if (success) {
        ...
        setSentCode(code); // nếu muốn sử dụng mã xác thực ở bước 4
        setStep(4);
      } else {
        setEmailError(message); // hiện lỗi bên dưới form
      }
    } catch (error) {
      ...
      <div className="step-content">
        ...
        {step === 3 && (
          <form onSubmit={handleSubmitEmail}>
            ...
            {/* ✅ Thêm đoạn này hiển thị message */}
            {emailStatus === false && (
              <p
                className={emailStatus.success ? "text-success" : "text-error"}
