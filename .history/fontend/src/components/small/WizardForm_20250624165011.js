...
const WizardForm = () => {
  ...
  const handleChange = (section) => (e) => {
    ...
    if (section === "account" && name === "email") {
      ...
      setEmailError(isValid || value === "" ? "" : "Email không hợp lệ");
    }
  ...
  const isStepOneComplete = () => {
    ...
    return (
      email.trim() !== "" &&
      isValidEmail(email) &&      // ✅ Email đúng định dạng
      displayName.trim() !== "" &&
      password.trim() !== ""
    );
  };
  ...
  const handleResendCode = () => {
    ...
    setResendCooldown(120); // 3 phút = 180 giây
  };
            ...
            {["email", "displayName", "password"].map((field) =>
              field === "password" ? (
                ...
                <input
                  ...
                  type="text"
                  placeholder={field === "displayName" ? "Tên hiển thị" : ""}
                  name={field}
