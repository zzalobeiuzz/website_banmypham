...
const RegistrationForm = () => {
  ...
  return (
    <div style={{ maxWidth: '500px', margin: '50px auto', fontFamily: 'Arial, sans-serif' }}>
      ...
      {/* Bước 1 */}
      <div
        style={{ ...circleStyle(step >= 1, step === 1), zIndex: 1 }}
        onClick={() => setStep(1)}
      >
        {step > 1 ? '✓' : '1'}
      </div>
      {/* Bước 2 */}
      <div
        style={{ ...circleStyle(step >= 2, step === 2), zIndex: 1 }}
        onClick={() => setStep(2)}
      >
        {step > 2 ? '✓' : '2'}
      </div>
      {/* Bước 3 */}
      <div
        style={{ ...circleStyle(step >= 3, step === 3), zIndex: 1 }}
        onClick={() => setStep(3)}
      >
        3
      </div>
...
const stepLineStyle = {
  position: 'absolute',
  top: '50%',
  left: 'calc(50% - 100px)', // Tùy chỉnh độ dài đường line
  width: '200px',            // Chiều dài tổng của line giữa các bước
  height: '4px',
  backgroundColor: '#00CFA1',
  zIndex: 0,
};
