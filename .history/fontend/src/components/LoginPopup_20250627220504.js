...
const LoginPopup = ({ toggleLoginPopup, onLoginSuccess }) => {
  ...
  const [serverCode, setServerCode] = useState(""); // ✅ Mã từ server trả về
  ...
  const handleSendCodeForm = async (e) => {
    ...
    try {
      ...
      if (response.success) {
        ...
        setServerCode(response.code);        // ✅ lưu mã server trả về
        setTimeout(() => nextStep(), 1000);  // sang bước xác thực
      }
    } catch (err) {
