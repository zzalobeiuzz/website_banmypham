...
const Select = ({ title }) => {
  ...
  //================ ✅ Gọi API và cập nhật sản phẩm
  useEffect(() => {
    const fetchProducts = async () => {
      let url = null;
      if (title === "Flash Sale") {
        url = "http://localhost:5000/api/user/products/sale";
      } else if (title === "Sản phẩm hot") {
        url = "http://localhost:5000/api/user/products/hot";
      }

      if (!url) return;

      try {
        const data = await request("GET", url,{title});
      ...
      <div className="slide-top">
        <div className="slide-title d-flex align-items-center gap-2">
          <a href="/" className="d-flex align-items-center gap-2">
            <img className="img-fluid" src={icon_select} alt="Icon sản phẩm" />
            <h2>{title}</h2>
          </a>
        </div>
