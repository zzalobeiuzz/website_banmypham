...
const FlashSale = () => {
  ...
  const handleMouseUp = (e) => {
    ...
    // Hủy bỏ các sự kiện khi thả chuột
    ...
    const itemsScrolled = Math.round(Math.abs(deltaX) / ITEM_WIDTH); 
    const remainder = Math.abs(deltaX) % ITEM_WIDTH;
  
    const totalItemsToMove = remainder >= ITEM_WIDTH / 2 ? itemsScrolled + 1 : itemsScrolled;
    const finalTranslateX = currentTranslateX.current - direction * totalItemsToMove * ITEM_WIDTH;
    ...
    // Tính toán các item đang hiển thị
    const newActiveIndexes = [];
    const activeIndexStart = Math.floor(Math.abs(clampedTranslateX) / ITEM_WIDTH); // Chỉ số của item đầu tiên đang hiển thị
  
    // Cập nhật activeIndexes dựa trên giá trị của translateX
