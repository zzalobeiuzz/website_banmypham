...
importuseHttp from "../../../../hooks/useHttp";
...
const Select = ({ title }) => {
  ...
  // ✅ Gọi API dựa vào title truyền vào
  const {
    ...
    if (fetchedProducts) {
      const updateCountdown = () => {
        ...
        const updated = fetchedProducts.map((product) => {
          ...
          const formatted = `Còn ${days} ngày ${String(hours).padStart(
            2,
            "0"
  ...
  //=============================✅ Animation phần cuộn thanh ngang sản phẩm
  const handleMouseMove = useCallback(
    (e) => {
      if (e.buttons !== 1) return;
      const deltaX = e.clientX - startX.current;
      const maxTranslateX = -(
        products.length * ITEM_WIDTH -
        VISIBLE_COUNT * ITEM_WIDTH
      );
              ...
              {products.map((product, index) => (
                ...
                >
                  <a href="/a" className="product-template">
                    {/* Nếu có giảm giá (title Flash Sale hoặc có sale_price) thì hiển thị giảm giá */}
                    ...
                    {/*Nếu là tiltle là Flash Sale thì hiện border của img ngược lại border none */}
                    {title === "Flash Sale" ? (
                      <img
                    ...
                    <div className="product-price px-2">
                      {/* Nếu có giá khuyến mãi thì hiển thị, ngược lại chỉ hiển thị giá gốc */}
                      {product.sale_price ? (
                        <>
                    ...
                    <div className="product-brand px-2">
                      {product.SupplierID}
                    </div>
                    ...
                    {/* ⏳ Chỉ hiển thị countdown nếu là "Flash Sale" */}
                    {title === "Flash Sale" && (
