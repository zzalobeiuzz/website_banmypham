...
const Select = ({ title }) => {
  ...
  const { request } = useHttp(); // ✅ gọi hook useHttp
  ...
  useEffect(() => {
    ...
    const updateCountdown = () => {
      ...
      const updated = fetchedProducts.map((product) => {
        ...
        const formatted = `Còn ${days} ngày ${String(hours).padStart(2, "0")} : ${String(
          minutes
    ...
    updateCountdown(); // ban đầu
    const timerId = setInterval(updateCountdown, 1000); // mỗi giây
  ...
  const handleMouseUp = (e) => {
    ...
    if ((currentTranslateX.current === 0 && direction > 0) || (currentTranslateX.current === maxTranslateX && direction < 0)) {
      return;
    }
    ...
    scrolledItemsCount.current = Math.max(Math.min(scrolledItemsCount.current, products.length - VISIBLE_COUNT), 0);
  ...
  return (
    <div className="section-flash-mobile d-block slide-template bg-white mb-4 pt-1">
      ...
      <div className="slide-main">
        <div className="slide-template-slide" ref={containerRef} onMouseDown={handleMouseDown}>
                ...
                >
                  <a href="/a" className="product-template">
                    ...
                    <div className="product-price px-2">
                      {product.sale_price ? (
                        <>
                          <div className="public-price">{product.sale_price.toLocaleString("vi-VN")}đ</div>
                          <div className="origin-price">{product.Price.toLocaleString("vi-VN")}đ</div>
                        </>
                      ) : (
                        <div className="public-price">{product.Price.toLocaleString("vi-VN")}đ</div>
                      )}
                    </div>
