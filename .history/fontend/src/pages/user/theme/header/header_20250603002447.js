...
const Header = () => {
  ...
  useEffect(() => {
    const handleScroll = () => {
      ...
      // ✅ Nếu đang cuộn xuống và chưa fixed, và trong vùng cần auto scroll
      if (
        isScrollingDown &&
        !isFixed &&
        currentScrollY >= 20 &&
        ...
        // Tránh set lại nếu đã gần 145
        window.scrollTo({ top: 145, behavior: "smooth" });
        setIsFixed(true);
      }
  
      // ✅ Nếu cuộn lên và quay về đầu trang
      if (!isScrollingDown && currentScrollY < 100 && isFixed) {
        setIsFixed(false);
      }
  
      // ✅ Hiển thị dịch vụ khách hàng khi ở gần đầu trang
