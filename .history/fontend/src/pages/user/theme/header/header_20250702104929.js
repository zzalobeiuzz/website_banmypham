...
const Header = () => {
  ...
  useEffect(() => {
    ...
    const fetchCategories = async () => {
      try {
        const res = await request("GET", "http://localhost:5000/api/user/product/loadCategory");
        if (res.success) {
          setCategories(res.data); // hoặc res.data.data tùy backend trả
        }
      } catch (error) {
                ...
                {user ? (
                  ...
                  <div className="login-info d-flex align-items-center gap-3">
                    <div className="login-button">
                      ...
                      <span>  {showCustomService ? `Xin chào, ${user.name}` : user.name}
                      </span>
                    </div>
                    {/* Nút đăng xuất */}
                    <button
