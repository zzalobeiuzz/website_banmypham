...
const Header = () => {
  ...
  useEffect(() => {
    const handleScroll = () => {
      ...
      // Kiểm tra nếu người dùng lướt xuống
      const isScrollingDown = currentScrollY > lastScrollY;
  
      if (isScrollingDown && !isFixed && currentScrollY >= 20 && currentScrollY < 145) {
        setIsFixed(true);
        window.scrollTo({ top: 145, behavior: "smooth" });
      } else if (currentScrollY < 40) {
        // Trở về đầu trang thì bỏ fixed
        setIsFixed(false);
      }
  
      // Phần hỗ trợ khách hàng hiển thị khi scrollY <= 40
      setShowCustomService(currentScrollY <= 40);
  
      // Cập nhật scrollY hiện tại
