...
const Header = () => {
 ...
 useEffect(() => {
  const handleScroll = () => {
    ...
    // Nếu kéo xuống và vượt qua y = 30, và chưa fixed
    if (currentScrollY > 30 && !isFixed) {
      setIsFixed(true);
      setShowCustomService(false);

      // Cuộn xuống y = 150 chỉ khi chưa đạt tới đó
      if (currentScrollY < 150) {
        window.scrollTo({ top: 150, behavior: "smooth" });
      }
    }

    // Nếu kéo lên và trở lại y = 150 trở lên (tức scrollY <= 150)
    if (currentScrollY <= 160 && isFixed) {
      setIsFixed(false);
