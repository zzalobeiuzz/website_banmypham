...
const Header = () => {
  ...
  const smoothScrollTo150 = () => {
    const startY = window.scrollY;
    const distance = 150 - startY;
    ...
    const step = (currentTime) => {
      ...
      const ease = progress * (2 - progress); // easing đơn giản
  ...
  useEffect(() => {
    const handleScroll = () => {
      ...
      if (isScrollingDown && currentScrollY > 30 && !isFixed) {
        setIsFixed(true);
      ...
      if (!isScrollingDown && currentScrollY <= 150 && isFixed) {
        setIsFixed(false);
