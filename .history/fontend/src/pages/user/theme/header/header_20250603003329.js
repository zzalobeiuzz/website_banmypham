...
const Header = () => {
  ...
  useEffect(() => {
    const handleScroll = () => {
      ...
      // Nếu đang cuộn xuống và nằm trong khoảng 10 -> 145 thì đẩy lên 145 và bật fixed
      if (isScrollingDown && currentScrollY >= 10 && currentScrollY < 145) {
        window.scrollTo({ top: 145, behavior: "smooth" });
        setIsFixed(true);
      }
  
      // Cập nhật isFixed chỉ dựa trên vị trí y
      if (currentScrollY >= 100) {
        setIsFixed(true);
      } else if (currentScrollY < 130) {
        setIsFixed(false);
      }
  
      // Hỗ trợ khách hàng chỉ hiện khi scrollY <= 40
      setShowCustomService(currentScrollY <= 40);
