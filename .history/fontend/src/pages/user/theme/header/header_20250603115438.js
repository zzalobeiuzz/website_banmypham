...
const Header = () => {
  ...
  useEffect(() => {
    const handleScroll = throttle(() => {
      ...
      // Khi scroll vượt header.height, bật fixed
      if (scrollPosition > header.offsetHeight) {
        if (!isFixed) setIsFixed(true);
        setShowCustomService(false); // ẩn hỗ trợ khách hàng khi fixed
        // Thêm margin cho dropdown menu khi fixed
        if (dropdownRef.current) dropdownRef.current.style.marginTop = "60px";
      } else {
        if (isFixed) setIsFixed(false);
        setShowCustomService(true); // hiện hỗ trợ khi không fixed
        if (dropdownRef.current) dropdownRef.current.style.marginTop = "0px";
      }
    }, 100);
