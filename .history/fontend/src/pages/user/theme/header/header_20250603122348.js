...
const Header = () => {
  ...
  useEffect(() => {
    const handleScroll = throttle(() => {
      ...
      if (!header || !logo || !dropdown) return;
  
      if (scrollPosition > header.offsetHeight) {
        setIsFixed(true);
        setLogoSrc(require("../../assets/images/logo2.png"));
        dropdown.style.marginTop = "60px";
      } else {
        setIsFixed(false);
        setLogoSrc(require("../../assets/images/logo1.png"));
        dropdown.style.marginTop = "0px";
      }
    }, 100); // chạy tối đa 1 lần mỗi 100ms
  
    window.addEventListener("scroll", handleScroll);
    return () => {
      window.removeEventListener("scroll", handleScroll);
      handleScroll.cancel(); // huỷ throttle khi component unmount
    };
  }, []);
