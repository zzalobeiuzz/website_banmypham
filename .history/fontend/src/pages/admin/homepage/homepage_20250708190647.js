...
const Homepage = () => {
  ...
  useEffect(() => {
    const checkAdmin = async () => {
      ...
      try {
        await request("GET", `${API_BASE}/api/admin`, null, { Authorization: `Bearer ${token}` });
      } catch (error) {
        if (error.status === 401 && error.message === "Token không hợp lệ") {
          try {
            ...
            await request("GET", `${API_BASE}/api/admin`, null, { Authorization: `Bearer ${newAccessToken}` });
          } catch {
  ...
  const MenuSection = ({ title, items }) => {
    ...
    return (
      <li className={`menu-section ${isOpen ? "open" : ""}`}>
        ...
        <ul className="list-child" ref={contentRef} style={{ maxHeight: height }}>
          {items.map((item, idx) => (
  ...
  return (
    <div className="homepage">
      <div className={`menu-panel ${isCollapsed ? "collapsed" : ""}`}>
