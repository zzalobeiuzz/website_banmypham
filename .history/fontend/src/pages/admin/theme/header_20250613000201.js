  ...
  const toggleSearch = () => {
    if (isActive) {
      ...
      setTimeout(() => {
        setIsClosing(false);
        setIsActive(false);
      }, 300); // Thời gian khớp animation CSS
    } else {
      setIsActive(true);
    }
  };
    ...
    <header>
      <div className="container">
        ...
        <div className="search-container">
          <div className={`search-wrapper ${isActive ? "show" : ""} ${isClosing ? "closing" : ""}`}>
            <div className="search">
              <input
                type="text"
                placeholder="Tìm kiếm..."
                className={`input_search ${isClosing ? "closing" : ""} ${!isActive && !isClosing ? "hidden" : ""}`}
              />
