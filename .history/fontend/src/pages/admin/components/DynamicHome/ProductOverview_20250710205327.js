...
export const ProductOverview = ({ searchKeyword }) => {
    ...
    const [categories, setCategories] = useState([]); // 🗂️ Danh sách danh mục
    const [products, setProducts] = useState([]); // 📦 Danh sách sản phẩm
    const [selectedCategory, setSelectedCategory] = useState("Tất cả"); // 🔖 Danh mục được chọn

    // 🔄 State cho toggle filter & animation danh mục
    ...
    // ✅ State chọn sản phẩm & edit
    const [selectMode, setSelectMode] = useState(false); // 🟢 Bật/tắt chế độ chọn nhiều
    const [selectedProducts, setSelectedProducts] = useState([]); // ✔️ Danh sách sản phẩm được chọn
