...
const ProductOverviewComponent  = ({ searchKeyword }) => {
    ...
    const [products, setProducts] = useState([]);            // 📦 Lưu danh sách sản phẩm từ API đổ vào bảng
    const [originalProducts, setOriginalProducts] = useState([]);  // 📦 Lưu danh sách sản phẩm từ API
    ...
    const handleEditOrSave = async () => {
        if (editMode) { //đang ở chế độ chỉnh sửa
            const confirmSave = window.confirm("Bạn có chắc chắn muốn lưu thay đổi?");
            if (!confirmSave) return;

            // ✅ Lọc ra danh sách sản phẩm mà người dùng đã chọn để chỉnh sửa
            const updatedProducts = products.filter((p) =>
                //Điều kiện lọc: Chỉ giữ lại những sản phẩm có ProductID nằm trong mảng selectedProducts.
