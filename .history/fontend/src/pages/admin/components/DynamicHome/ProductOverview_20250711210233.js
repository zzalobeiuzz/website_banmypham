...
const ProductOverviewComponent = ({ searchKeyword }) => {
    ...
    const handleCloseCategories = () => {
        ...
        const totalTime = categories.length * 100 + 400; // Thời gian hiệu ứng
    ...
    const handleEditOrSave = async () => {
        if (editMode) {
            ...
            try {
                await request("PUT", `${API_BASE}/api/admin/products/updateProducts`, updatedProducts, "Cập nhật sản phẩm");
        ...
        <div className="product-wrapper">
            ...
            <div className={`product-content ${filterOpen ? "open" : ""}`}>
                <div className="product-left">
                    <button onClick={handleToggleSelectMode}>{selectMode ? "Huỷ chọn" : "Chọn sản phẩm"}</button>
                    ...
                    <div className="filter-toggle-header" onClick={handleToggleFilter}>
                        {!filterOpen ? (
                            <>
                                <img src="/assets/icons/icons8-filter.gif" alt="filter icon" style={{ width: "24px", height: "24px" }} />
                                <span>Mở bộ lọc</span>
                            </>
                        ) : (
                    ...
                    {showFilterContent && <div className="filter-body"><p>Đây là các tùy chọn lọc...</p></div>}
                </div>
                                ...
                                {filteredProducts.map((product, index) => (
                                    <ul key={product.ProductID} className="list-unstyled row-data">
                                        ...
                                        <li className="list-image">
                                            <img src={`/assets/pictures/${product.Image}`} alt={product.ProductName} width="70" />
                                        </li>
