...
exports.resetPasswordHandler = async (req, res) => {
  try {
    ...
    // Lấy object verifications từ session
    ...
    if (!sessionData) {
      return res
        .status(400)
        .json({ success: false, message: "Không tìm thấy mã xác thực cho email này." });
    }
    ...
    await resetPassword({
      ...
      code,
      newPassword,
      sessionVerifications: req.session.verifications
    });

    // Xoá session key sau khi thành công
