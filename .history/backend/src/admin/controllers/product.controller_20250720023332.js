...
exports.handleProductDetail = async (req, res) => {
  try {
    ...
    // ✅ Truy cập vào object product bên trong
    const productId =result.product.id;
    const detailID = result.product.detailid;

    console.log("productId: " + productId);
    console.log("detailID: " + detailID);

    // 🟢 Bước 2: Lấy thêm chi tiết sản phẩm
    const detail = await productService.getProductDetail(detailID);

    // 🟢 Bước 3: Lấy thêm danh mục phụ nếu có
    const subCategory = await productService.getSubCategoryByProductId(productId);
