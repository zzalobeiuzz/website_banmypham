...
exports.addProduct = async (req, res) => {
  try {
    ...
    if (result.success && req.file) {
      // Bắt buộc đặt lại phần mở rộng file là .jpg
      const ext = ".jpg";

      // Lấy tên gốc của file (không bao gồm phần mở rộng), ví dụ: "hinh_san_pham"
      const rawName = path.basename(req.file.originalname, path.extname(req.file.originalname));

      // Tạo tên file mới: giữ nguyên tên gốc, thay đuôi thành .jpg
      const filename = `${rawName.replace(/\s+/g, "_")}${ext}`;

      // Đường dẫn lưu file vào thư mục uploads/assets/pictures
      ...
      // Ghi file ảnh từ bộ nhớ tạm vào ổ cứng
