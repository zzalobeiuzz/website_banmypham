...
exports.addProduct = async (req, res) => {
  try {
    ...
    // ✅ Nếu thêm sản phẩm thành công và có file ảnh được gửi lên
    if (result.success && req.file) {
      // Đặt lại phần mở rộng là .jpg
      const ext = ".jpg";
    
      // Lấy tên gốc của file (bỏ phần đuôi)
      const rawName = path.basename(req.file.originalname, path.extname(req.file.originalname));
    
      // Tạo tên file mới (giữ nguyên tên, đổi đuôi .jpg)
      const filename = `${rawName.replace(/\s+/g, "_")}${ext}`;
    
      // Đường dẫn lưu ảnh
      const savePath = path.join(__dirname, "../../../uploads/assets/", filename);
    
      // Nếu ảnh đã tồn tại thì bỏ qua lưu, log ra thông báo
      if (fs.existsSync(savePath)) {
        console.log("⚠️ Ảnh đã tồn tại, không cần lưu lại:", filename);
      } else {
      ...
      // Gán lại tên ảnh vào `req.body.Image` nếu bạn cần lưu vào DB
