...
exports.handleProductDetail = async (req, res) => {
  try {
    ...
    const product = await productService.checkProductExistence(code);
    if (!product) {
      return res.status(404).json({ success: false, message: "Không tìm thấy sản phẩm" });
    }

    const productId = product.id;
    const detailID = product.detail
    // 🟢 Bước 2: Lấy thêm chi tiết sản phẩm (nội dung mô tả, thông số, v.v...)
