...
exports.checkProductExists = async (productId) => {
  ...
  const result = await pool.request()
    ...
    .query(`
      SELECT 
        ...
        -- Thông tin SubCategory
        SC.SubCategoryID,
        SC.SubCategoryName,
        SC.CategoryID AS SubCategory_CategoryID,

        -- Thông tin Detail (nếu có)
        D.DetailID,
        D.Intro,
        D.Usage,
        D.Ingredients,
  ...
  if (result.recordset.length > 0) {
    return result.recordset[0]; // ✅ Đã chứa thông tin product + subcategory + detail
  } else {
    return null;
  }
};
