...
exports.addProductDB = async (product) => {
  try {
    // ✅ 1. Kiểm tra xem ProductID đã tồn tại trong bảng PRODUCT chưa
    ...
    // ✅ 3. Destructure các thuộc tính từ object product
    const {
      ProductID,
      ProductName,
      Price,
    ...
    // ✅ 4. Thêm dữ liệu vào bảng PRODUCT
    await pool.request()
      .input("ProductID", sql.VarChar(50), ProductID)
    ...
    // ✅ 5. Thêm dữ liệu vào bảng PRODUCT_DETAIL
    await pool.request()
    ...
    // ✅ 6. Thêm SUBCATEGORY nếu chưa tồn tại (tránh duplicate)
    await pool.request()
    ...
    // ✅ 7. Log và trả kết quả thành công
    console.log('✅ success: true, message: Thêm sản phẩm thành công');
    return {
      success: true,
      message: `Thêm sản phẩm ${ProductName} và detail thành công`,
    };
  } catch (err) {
    // ❌ 8. Bắt lỗi và log chi tiết
    console.log("❌ Lỗi addProductDB:", err);
    return {
      success: false,
