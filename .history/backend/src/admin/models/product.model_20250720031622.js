...
exports.checkProductExists = async (productId) => {
  try {
    ...
    const result = await pool.request()
      ...
      SC.SubCategoryID,              -- 🔸 Mã danh mục phụ
      SC.SubCategoryName,            -- 🔸 Tên danh mục phụ
      SC.CategoryID AS SubCategory_CategoryID, -- 🔸 Mã danh mục cha (của danh mục phụ), đổi tên để tránh trùng
    
      -- 🟡 Thông tin chi tiết sản phẩm (Detail)
      D.DetailID,        -- 🔹 Mã chi tiết
      D.Intro,           -- 🔹 Giới thiệu sản phẩm
      D.Usage,           -- 🔹 Công dụng
      D.Ingredients,     -- 🔹 Thành phần
      D.Instructions,    -- 🔹 Hướng dẫn sử dụng
      D.OtherInfo        -- 🔹 Thông tin khác (nếu có)
    
    FROM PRODUCT P
    -- 🔗 Kết hợp với bảng Sub_Category theo SubCategoryID để lấy thông tin danh mục phụ
    LEFT JOIN Sub_Category SC ON P.SubCategoryID = SC.SubCategoryID
    
    -- 🔗 Kết hợp với bảng DETAIL theo DetailID để lấy thông tin chi tiết sản phẩm
    LEFT JOIN DETAIL D ON P.DetailID = D.DetailID
    
    -- 🔍 Điều kiện lọc: chỉ lấy sản phẩm có ProductID tương ứng
    ...
    return {
      ...
      subCategory: SubCategoryID ? {
        ...
        categoryId: SubCategory_CategoryID,
      } : null,
      detail: DetailID ? {
        detailId: DetailID,
        intro: Intro,
