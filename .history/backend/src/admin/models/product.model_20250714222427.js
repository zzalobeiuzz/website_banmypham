...
exports.create = async (product) => {
  ...
  await pool.request()
    ...
    .input("CreatedAt", sql.Date, CreatedAt ?? new Date())
    .input("SupplierID", sql.NVarChar(100), SupplierID ?? null)
    .input("Image", sql.NVarChar(sql.MAX), Image ?? null)
    .input("Intro", sql.NVarChar(sql.MAX), Intro ?? null)
    ...
    .query(`
      INSERT INTO PRODUCT
      (ProductID, ProductName, Description, IsHot, Type, Price, CategoryID, SubCategoryID, StockQuantity, CreatedAt, SupplierID, Image, Intro, Usage, Ingredients, Instructions)
      VALUES
      (@ProductID, @ProductName, @Description, @IsHot, @Type, @Price, @CategoryID, @SubCategoryID, @StockQuantity, @CreatedAt, @SupplierID, @Image, @Intro, @Usage, @Ingredients, @Instructions)
    `);
...
exports.addProductDB = async (product) => {
  ...
  await pool.request()
    ...
    .query(`
      INSERT INTO PRODUCT
      (ProductID, ProductName, Price, Type, CategoryID,  StockQuantity, SupplierID, IsHot, Intro, Usage, Ingredients, Instructions, Image, DetailID)
      VALUES
      (@ProductID, @ProductName, @Price, @Type, @CategoryID, @SubCategoryID, @StockQuantity, @SupplierID, @IsHot, @Intro, @Usage, @Ingredients, @Instructions, @Image, @DetailID)
    `);
