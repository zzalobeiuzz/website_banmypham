...
export const handleHtmlImagesBatch = async (htmlArr) => {
    ...
    try {
        ...
        if (!data.newUrls || !Array.isArray(data.newUrls)) {
            console.log("❗ Server không trả về danh sách URL mới hợp lệ");
            return htmlArr;
        }

        // ✅ Cập nhật src, dùng counter riêng
        let globalIndex = 0;

        const updatedHtmlArr = htmlArr.map((_, i) => {
            const $ = cheerios[i];
            $("img").each((_, img) => {
                if (data.newUrls[globalIndex]) {
                    $(img).attr("src", data.newUrls[globalIndex]);
                    globalIndex++;
                }
            });
            return $.html();
